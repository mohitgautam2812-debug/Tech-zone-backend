...
Route::middleware(['auth'])->group(function () {
    Route::get('/orders', [OrderController::class, 'index'])
        ->name('orders')->middleware('permission:view_orders');
    Route::get('/orders/pending', [OrderController::class, 'pending'])
        ->name('orders.pending')->middleware('permission:pending_orders');
    Route::get('/orders/delivered', [OrderController::class, 'delivered'])
        ->name('orders.delivered')->middleware('permission:delivered_orders');
    Route::get('/orders/cancelled', [OrderController::class, 'cancelled'])
        ->name('orders.cancelled')->middleware('permission:cancelled_orders');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/users', [UserController::class, 'index'])
        ->name('users')->middleware('permission:view_users');
    Route::get('/users/agents', [UserController::class, 'agents'])
        ->name('users.agents')->middleware('permission:view_agents');
    Route::get('/add-user', [UserController::class, 'create'])
        ->name('users.create')->middleware('permission:add_users');
    Route::post('/users/store', [UserController::class, 'store'])
        ->name('users.store')->middleware('permission:add_users');
    Route::post('/users/status/{id}', [UserController::class, 'updateStatus'])
        ->name('users.status')->middleware('permission:view_users');
    Route::put('/users/{user}', [UserController::class, 'update'])
        ->name('users.update')->middleware('permission:view_users');
});

Route::middleware(['auth'])->group(function () {
    Route::get('/inquiries', [InquiryController::class, 'index'])
        ->name('inquiries')->middleware('permission:view_inquiries');
    Route::post('/inquiries', [InquiryController::class, 'store'])->name('inquiries.store');
});

Route::middleware(['auth'])->group(function () {
    Route::get('/reviews', [ReviewController::class, 'index'])
        ->name('reviews')->middleware('permission:view_reviews');
    Route::post('/reviews', [ReviewController::class, 'store'])->name('reviews.store');
});

Route::middleware(['auth'])->group(function () {
    Route::get('/settings', [ProfileController::class, 'settings'])
        ->name('settings')->middleware('permission:settings');
    Route::post('/settings/update', [ProfileController::class, 'updateProfile'])->name('settings.update');
});

Route::middleware(['auth'])->group(function () {
    Route::get('/roles', [RoleController::class, 'index'])
        ->name('roles.index')->middleware('permission:manage_roles');
    Route::post('/roles/store', [RoleController::class, 'store'])
        ->name('roles.store')->middleware('permission:manage_roles');
    Route::get('/roles/{id}/edit', [RoleController::class, 'edit'])
        ->name('roles.edit')->middleware('permission:manage_roles');
    Route::post('/roles/{id}/update', [RoleController::class, 'update'])
        ->name('roles.update')->middleware('permission:manage_roles');
    Route::delete('/roles/{id}', [RoleController::class, 'destroy'])
        ->name('roles.delete')->middleware('permission:manage_roles');
});

// categories resource auto-creates: categories.index, categories.create, categories.store,
// categories.show, categories.update, categories.destroy — so NO manual categories.destroy below
...
Route::middleware(['auth'])->group(function () {
    ...
    })->name('wishlist')->middleware('permission:wishlist');
});

Route::middleware(['auth'])->group(function () {
    Route::get('/my-orders', function () {
        return view('my_orders');
    })->name('my_orders')->middleware('permission:my_orders');
});
...
Route::get('/footer-settings', [SettingController::class, 'index'])->name('settings.index');
Route::post('/settings', [SettingController::class, 'store'])->name('settings.store');
Route::get('/invoice/{id}', [OrderController::class, 'downloadInvoice'])->name('invoice');

Route::middleware(['auth'])->group(function () {

    Route::get('/admin/manage-pages', [PageController::class, 'managePages'])->name('manage.pages');
    Route::get('/admin/pages/home', [PageController::class, 'home'])->name('pages.home');
    ...
    // categories edit/delete — use unique names to avoid conflict with resource routes
...
Route::prefix('admin')->middleware(['auth'])->group(function () {
    ...
    Route::delete('blogs/{id}', [BlogController::class, 'adminDelete'])->name('admin.blogs.delete');
    Route::patch('blogs/{id}/toggle', [BlogController::class, 'adminToggle'])->name('admin.blogs.toggle');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/contact-inquiries', [InquiryContactController::class, 'index'])
        ->name('contact.inquiries')->middleware('permission:view_contact_inquiries');
...
Route::get('/admin/pages/about', [AboutPageController::class, 'manage'])->name('pages.about');
